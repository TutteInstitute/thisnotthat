...
class SearchPane(pn.reactive.Reactive):
    ...
    def _query_style_change(self, event):
    ...
    def _run_query(self, event):
        ...
        if len(self.query_box.value) == 0:
            ...
            try:
                ...
                for col in self.columns_to_search.value or self.data:
                    if hasattr(self.data[col], "str"):
                        new_indices = np.where(self.data[col].str.contains(self.query_box.value, regex=False))
                            [0].tolist()
                        indices.extend(new_indices)
                if len(indices) == 0:
                    self.warning_area.alert_type = "warning"
                    self.warning_area.object = f"No matches found for search string {self.query_box.value}!"
            ...
            try:
                ...
                for col in self.columns_to_search.value or self.data:
                    if hasattr(self.data[col], "str"):
                        new_indices = np.where(self.data[col].str.contains(self.query_box.value, regex=True))
                            [0].tolist()
                        indices.extend(new_indices)
                if len(indices) == 0:
                    self.warning_area.alert_type = "warning"
            ...
            try:
                self.selected = self.data.reset_index().query(self.query_box.value).index.tolist()
    ...
    def __init__(self, raw_dataframe):
        ...
        self.query_style_selector = pn.widgets.RadioButtonGroup(
            name="Query type",
            options=["String search", "Regex", "Pandas query"], button_type="primary",
        )
