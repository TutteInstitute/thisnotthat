...
class JointWordCloudSummarizer:
    ...
    def summarize(self, selected: Sequence[int], width: int = 600, height: int = 600) -> LayoutDOM:
        ...
        for (word, count), font_size, position, orientation, color in word_cloud.layout_:
            fig.text(x_offset=[3 * position[1]], y_offset=[-3 * position[0]], [word], text_alpha=0.8, text_font={"value": "DroidSansMono"}, text_font_size=f"{font_size}px", text_color=color, angle=90 if orientation == "ROTATE_90" else 0, angle_units="deg")
